...
def combine_pickles(file1, file2, file3, file4):
    # df_y is the same 
    df_X1, df_y = load_pickle(file1)
    df_X2, df_y = load_pickle(file2)
    df_X3, df_y = load_pickle(file3)
    df_X4, df_y = load_pickle(file4)
    
    new_X = []
    for i in range(len(df_y)):
        new_X.append(df_X1[i] + df_X2[i] + df_X3[i] + df_X4[i])
...
def main():
    ...
    df_X, df_y = combine_pickles(file1="../pickles/text_embeddings.pkl", 
        ...
        file4="../pickles/handcrafted_features.pkl")
